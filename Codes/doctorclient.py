...
class DoctorClient(EasyFrame):
    ...
    def _init_(self):
        ...
        self.drLabel = self.addLabel(text = "Please enter your name to connect.",
                                    row = 0, column = 0, columnspan = 2, background= DoctorClient.COLOR)
        self.ptField = self.addTextField(text = " ", row = 1, column = 0, columnspan = 2, width = 50)
        self.sendBtn = self.addButton(row = 2, column = 0, text = "Send reply", command = self.sendReply, state= "disabled")
        self.connectBtn = self.addButton(row = 2, column = 1, text = "Connect", command= self.connect)
